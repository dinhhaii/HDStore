...
module.exports = {
    ...
    allWithDetail: () => {
        return db.load('select c.id, c.name, c.createddate, count(p.id) as totalproduct from category c left join product p on c.id = p.idcategory group by c.id, c.name, c.createddate');
    },
